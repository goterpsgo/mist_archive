...
start() {
	    ...
	    echo "$UWSGI_BIN --py-autoreload 1 --master --socket=/tmp/mist_app.sock --pidfile=/tmp/mist_app.pid --module=wsgi --honour-stdin --chdir=$APP_HOME --threads=1 --virtualenv=$ENV_HOME --die-on-term --uid $USER --gid $USER"
            $UWSGI_BIN --py-autoreload 1 --master --socket=/tmp/mist_app.sock --pidfile=/tmp/mist_app.pid --module=wsgi --honour-stdin --chdir=$APP_HOME --threads=1 --virtualenv=$ENV_HOME --die-on-term --uid $USER --gid $USER --logto=/opt/mist/mist_base/logs/mist_app.log > /dev/null 2>&1 &
		fi
